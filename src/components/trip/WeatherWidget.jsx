  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 py-3 px-1 mb-3">
        <div className="w-3.5 h-3.5 border border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(200,162,124,0.6)', borderTopColor: 'transparent' }} />
    ...
    <div className="mb-5 rounded-[18px] overflow-hidden" style={{ background: 'rgba(255,255,255,0.45)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.55)' }}>
      <div className="flex items-center justify-between px-4 pt-3.5 pb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setExpanded(false)} className="opacity-50 hover:opacity-80 transition-opacity">
            <ChevronUp className="h-3.5 w-3.5" style={{ color: '#9A8A7A' }} />
          </button>
        </div>
      </div>
      ...
      <div className="px-4 pb-3">
        {weather.days.map((d, i) => {
          ...
          return (
            <div key={i}>
              {i > 0 && <div style={{ height: 1, background: 'rgba(200,180,160,0.18)' }} />}
